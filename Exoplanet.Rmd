...
```{r}
KOI_table_log = data_log   # dataset obtained after performing a logtransform of the numerical features
# and capping some of those features exhibiting many outliers
KOI_table_log_f = data_log_f # after removing 2 uninformative features that have many outliers
KOI_table_capped = data_capped  #  after capping the values of the outliers at the 95% quantile 


#### To obtain the datasets wo the incertitudes :

KOI_table_log_wo_incertitudes <- KOI_table_log %>% dplyr::select(-all_of(column_incertitudes))

KOI_table_capped_wo_incertitudes <- KOI_table_capped %>% dplyr::select(-all_of(column_incertitudes))
