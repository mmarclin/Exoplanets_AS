...
```{r}
train_index.wo_log <- createDataPartition(KOI_table.wo_incertitudes.log$koi_disposition, p = 0.7, list = FALSE)
train_set.wo_log <- KOI_table.wo_incertitudes[train_index, ]
temp_set.wo_log  <- KOI_table.wo_incertitudes[-train_index, ]

val_index.wo_log <- createDataPartition(temp_set.wo_log$koi_disposition, p = 0.5, list = FALSE)
val_set.wo_log <- temp_set.wo_log[val_index.wo, ]
test_set.wo_log <- temp_set.wo_log[-val_index.wo, ]

train_set.wo_log$koi_disposition <- as.factor(train_set.wo_log$koi_disposition)
val_set.wo_log$koi_disposition <- as.factor(val_set.wo_log$koi_disposition)
test_set.wo_log$koi_disposition <- as.factor(test_set.wo_log$koi_disposition)

# LOG transformé sans incertitudes
train_index.log <- createDataPartition(KOI_table.wo_incertitudes.log$koi_disposition, p = 0.7, list = FALSE)
train_set.log <- KOI_table.wo_incertitudes.log[train_index.log, ]
temp_set.log  <- KOI_table.wo_incertitudes.log[-train_index.log, ]

val_index.log <- createDataPartition(temp_set.log$koi_disposition, p = 0.5, list = FALSE)
val_set.log <- temp_set.log[val_index.log, ]
test_set.log <- temp_set.log[-val_index.log, ]

train_set.log$koi_disposition <- as.factor(train_set.log$koi_disposition)
val_set.log$koi_disposition <- as.factor(val_set.log$koi_disposition)
test_set.log$koi_disposition <- as.factor(test_set.log$koi_disposition)
...
# Jeu sans incertitudes et sans outliers (log_f)
train_set.log_f <- KOI_table.log_f[train_index.log_f, ]
temp_set.log_f  <- KOI_table.log_f[-train_index.log_f, ]

val_index.log_f <- createDataPartition(temp_set.log_f$koi_disposition, p = 0.5, list = FALSE)
val_set.log_f <- temp_set.log_f[val_index.log_f, ]
test_set.log_f <- temp_set.log_f[-val_index.log_f, ]

# Jeu log transformé sans incertitudes
train_set.log <- KOI_table.wo_incertitudes.log[train_index.log, ]
temp_set.log  <- KOI_table.wo_incertitudes.log[-train_index.log, ]
...
## 2)b) without incertitudes

```{r}
run_lda_analysis(train_set.log, test_set.log, "LDA Projection - log (no uncertainties)")
...
Probabilites + ROC curve
```{r}
library(pROC)
```
```{r}
val_probs <- predict(svm_up, newdata = val_set, type = "prob")
roc_obj <- roc(val_set$koi_disposition, val_probs[, "1"])
plot(roc_obj)
auc(roc_obj)
```
...
```{r}
svm_preds_wo <- predict(svm_model_wo, newdata = val_set.wo)
