...
# Ensure koi_disposition is a factor
KOI_table$koi_disposition <- as.factor(KOI_table$koi_disposition)

# Subset the features + class
selected_features <- KOI_table[, c("koi_period", "koi_prad", "koi_duration", "koi_teq", "koi_disposition")]

# Plot
ggpairs(
  selected_features,
  aes(color = koi_disposition, alpha = 0.5),
...
### Pairplot for some features
# Subset the features + class
selected_features <- KOI_table.log_f[, c("koi_period", "koi_prad", "koi_duration", "koi_teq", "koi_disposition")]
...
val_index.wo <- createDataPartition(temp_set.wo$koi_disposition, p = 0.5, list = FALSE)
...
```
incertitudes are prevalent for prediction
then we have prad, impact, teq, steff
...
varImp(svm.wo.log)
```
importance: prad, impact, teq, insol, steff
...
varImp(svm.pca)
```
importance: comp2,comp4, comp1, comp6
...
##### Without incertitudes
```{r}
tree_model_wo_log <- rpart(koi_disposition ~ ., data = train_set.wo.log, method = "class")
summary(tree_model_wo_log)
tree_pred_wo_log <- predict(tree_model_wo_log, newdata = val_set.wo.log, type = "class")
table(Predicted = tree_pred_wo_log, Actual = val_set.wo.log$koi_disposition)
```
```{r}
...
acc_tree_wo_log <- mean(tree_pred_wo_log == val_set.wo.log$koi_disposition)
 
# comparison table
accuracy_table <- data.frame(
  model = c("All features", "Without uncertainties", "With log_f", "Without uncertainties + log_f"),
...
ggplot(accuracy_table, aes(x = model, y = accuracy, fill = model)) +
  geom_col(width = 0.6) +
  ylim(0, 1) +
  labs(title = "Decision Tree Accuracy Comparison",
       x = "Model",
       y = "Accuracy") +
...
cm4 <- confusionMatrix(tree_pred_wo_log, reference = val_set.wo.log$koi_disposition)
...
```
importance: incertitudes, next koi_prad
```{r}
## generate prediction and print the accuracy
...
rf_model_wo_log <- train(koi_disposition ~ .,  data = train_set.wo.log, method = "rf", trControl = control_wo_log, metric = 'Accuracy',
...
```
koi_prad is the most important feature, followed by koi_model_snr and koi_impact
```{r}
rf_preds_wo_log <- predict(rf_model_wo_log, newdata = val_set.log_f)
...
```{r}
rf_preds <- factor(rf_preds, levels = levels(val_set$target))
rf_preds_wo <- factor(rf_preds_wo, levels = levels(val_set.wo$target))
rf_preds_log_f <- factor(rf_preds_log_f, levels = levels(val_set.log_f$target))
rf_preds_wo_log <- factor(rf_preds_wo_log, levels = levels(val_set.wo.log$target))

rf_preds <- factor(rf_preds, levels = levels(val_set$target))

# Crea le confusion matrix
cm1 <- confusionMatrix(rf_preds, val_set$target)
cm2 <- confusionMatrix(rf_preds_wo, val_set.wo$target)
cm3 <- confusionMatrix(rf_preds_log_f, val_set.log_f$target)
cm4 <- confusionMatrix(rf_preds_wo_log, val_set.wo.log$target)
get_metrics <- function(cm) {
  acc <- cm$overall["Accuracy"]
  sens <- cm$byClass["Sensitivity"]
  spec <- cm$byClass["Specificity"]
...
# Crea il data frame per il plot
performance_df <- data.frame(
  Metric = rep(c("Accuracy", "Sensitivity", "Specificity"), 4),
  Value = c(metrics1, metrics2, metrics3, metrics4),
  Model = rep(
    c("RF 1", "RF 2", "RF 3", "RF 4"),
    each = 3
  )
)
...
features <- KOI_table.wo_incertitudes.log[,-13]
dim(features)

features.exo <- KOI_table.wo_incertitudes.log[KOI_table.wo_incertitudes.log$koi_disposition==1,-13]
features_exo <- scale(features.exo)
dim(features.exo)
```
```{r}
class <- as.factor(KOI_table.log_f$koi_disposition)
features <- KOI_table.log_f[,-which(names(KOI_table_log_f) == "koi_disposition")]
dim(features)
...
cluster_labels <- cutree(D.exo.ew, k = 2)
cluster_labels
```
...
plot_df <- features.exo
plot_df$Cluster <- as.factor(cluster_labels)  # make sure it's a factor

# Create the plot
ggplot(plot_df, aes(x = koi_teq, y = koi_prad, color = Cluster)) +
  geom_point(alpha = 0.7, size = 2) +
  labs(
    title = "Clusters of Exoplanets by Radius and Temperature",
    x = "Equilibrium Temperature (koi_teq)",
    y = "Planet Radius (koi_prad)",
    color = "Cluster"
  ) +
  theme_minimal()

# Create the plot
ggplot(plot_df, aes(x = ESI, y = koi_prad, color = Cluster)) +
  geom_point(alpha = 0.7, size = 2) +
  labs(
    title = "Clusters of Exoplanets by Radius and ESI",
    x = "ESI",
    y = "Planet Radius (koi_prad)",
    color = "Cluster"
  ) +
...
# Create the plot
ggplot(plot_df, aes(x = ESI, y = koi_teq, color = Cluster)) +
  geom_point(alpha = 0.7, size = 2) +
  labs(
    title = "Clusters of Exoplanets by Radius and ESI",
    x = "ESI",
...
ggplot(plot_df, aes(x = koi_period, y = koi_teq, color = Cluster)) +
  geom_point(alpha = 0.7, size = 2) +
  labs(
    title = "Clusters of Exoplanets by Radius and ESI",
    x = "Insolation Flux",
...
library(tidyr)
# Loadings
loadings <- pca$rotation[, 1:4]  # get loadings for PC1 to PC4
head(loadings)  # view top variables
...
# Plot
ggplot(loadings_long, aes(x = reorder(Variable, abs(Loading)), y = Loading, fill = PC)) +
  geom_bar(stat = "identity") +
  facet_wrap(~ PC, scales = "free_y") +
  coord_flip() +
  labs(title = "Loadings for First 4 Principal Components",
...
plot_ly(pca_3d_df, 
  ...
  layout(
    ...
    title = list(text = "3D PCA Plot Colored by Cluster")
  )
