...
ggplot(KOI_table_clean) +
  ...
  geom_density(alpha = 0.5) +
  scale_fill_manual(
    values = c("steelblue4", "indianred"),
    labels = c("False Positive", "Confirmed"),
    name = "Disposition"
  ) +
  theme_minimal() +
  labs(title = "Density of koi_steff by Exoplanet Disposition",
       x = "koi_steff",
       y = "Density")
...
feature_x <- "koi_steff_err1"
feature_y <- "koi_kepmag"
KOI_table_clean$koi_duration
...
## Non parametric Wilcoxon Test : no
...
barplot(eigenvalues, 
        ...
        ylim = c(0, max(eigenvalues) + 1))  # Extend y-axis for readability
#

plot(cumsum(eigenvalues))
# Plot of 2 first PC : 
pca_data <- data.frame(pca$x)  # First two PCs
pca_data$koi_disposition <- KOI_table_clean$koi_disposition  # Add the target variable

# Convert koi_disposition to a factor (if not already)
pca_data$koi_disposition <- as.factor(pca_data$koi_disposition)

ggplot(pca_data, aes(x = PC1, y = PC2, color = koi_disposition)) +
  geom_point(alpha = 0.7) + 
  scale_color_manual(values = c("steelblue4", "indianred")) +  # Confirmed = blue, False Positives = red
  theme_minimal() +
  labs(title = "PCA - First Two Principal Components", x = "PC1", y = "PC2", color = "Disposition") +
  theme(legend.position = "top")  # Add legend and position it on top
...
dim(KOI_table_PCA)
KOI_table_PCA = as.data.frame(KOI_table_PCA)
### Models prediction
library(caret)
...
# Standardize the quant features : 
target <- KOI_table_clean$koi_disposition

features <- KOI_table_clean %>% 
  select(-koi_disposition)  # Remove target variable

# Standardize features (Z-score scaling)
features_scaled <- scale(features)

KOI_table_scaled <- data.frame(features_scaled, koi_disposition = target)

# train test split 
train_index <- createDataPartition(KOI_table_scaled$koi_disposition, p = 0.8, list = FALSE)
train_data <- KOI_table_scaled[train_index, ]
test_data <- KOI_table_scaled[-train_index, ]

# train 1st model 
model <- glm(koi_disposition ~ ., data = train_data, family = binomial)
...
predicted_classes <- factor(predicted_classes, levels = levels(test_data$koi_disposition))
#Confusion matrix
confusionMatrix(predicted_classes, train_data$koi_disposition)
...
predicted_classes <- factor(predicted_classes, levels = levels(test_data$koi_disposition))
#Confusion matrix
confusionMatrix(predicted_classes, test_data$koi_disposition)

### train models and do PCA with scaled data
KOI_table_PCA = KOI_table_PCA[,1:3]

KOI_table_PCA$koi_disposition = KOI_table_clean$koi_disposition

# train test split 
train_index <- createDataPartition(KOI_table_PCA$koi_disposition, p = 0.8, list = FALSE)
train_data <- KOI_table_PCA[train_index, ]
test_data <- KOI_table_PCA[-train_index, ]
